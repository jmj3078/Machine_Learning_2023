...
# 폴더 이름을 바탕으로 각 폴더에 저장된 모든 이미지의 경로 이름을 호출하여 리스트로 저장
# jpg, jpeg, png를 모두 호출
for class_name in class_list:
    ...
    for img in img_list:
        ...
        h, w, c = image.shape
        # 이미지 자체에 오류가 있는 경우 오류 메세지 출력
        if image is None:
            print(
                f'Error in reading {img}')
            continue
...
print(f'Successfully Saved, path : {path_to_save}')